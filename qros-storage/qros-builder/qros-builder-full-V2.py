...
os.makedirs('builder', exist_ok=True)  # Create the directory if it doesn't exist
os.makedirs('builder/qr-videos', exist_ok=True)  # Create the directory if it doesn't exist
os.makedirs('builder/chunks', exist_ok=True)  # Create the directory if it doesn't exist
os.makedirs('builder/decoded', exist_ok=True)  # Create the directory if it doesn't exist
...
def compress_and_generate_base64_qr_images(file_path, chunk_size=1500):
    ...
    with open('builder/chunks/v86wasm.json', 'w') as json_file:
        json.dump({"chunks": chunks}, json_file)  # Save the chunks as an array within a JSON object

    os.makedirs('builder/qrs', exist_ok=True)  # Create the directory if it doesn't exist
    ...
    for i, chunk in enumerate(chunks):
        ...
        cv2.imwrite(f'builder/qrs/qr_{i:09d}.png', qr_img)  # Save each QR code as a PNG file

img_file_path = 'v86.wasm'
compress_and_generate_base64_qr_images(img_file_path)

# Add ffmpeg command to generate the video
os.system('ffmpeg -framerate 30 -i builder/qrs/qr_%09d.png -vf "scale=730:730,setsar=1" -an -c:v libx264 -pix_fmt yuv420p builder/qr-videos/v86.wasm.mp4')
...
video_capture = cv2.VideoCapture('builder/qr-videos/v86.wasm.mp4')
...
try:
    ...
    with open("builder/decoded/decoded_v86.wasm", "wb") as out_file:
        out_file.write(decompressed_data)
    print("Data decompressed and written to 'decoded_v86.wasm'.")
except Exception as e:
